...
class admin_jobs extends CI_Controller
{
    ...
    function view($value='')
    {
        if ($value!=''&& (ctype_digit($value))) {
            ...
            $data           = array('job' => $jobs[0],'company_name'=>$company_name);
    ...
    function approveJob($value='')
    {
        ...
        if ($value!=''&& (ctype_digit($value))) {
            ...
            if ($jobs[0]['approved']== '1') {
                $res = $this->admin_model->approveJob($job_id,array('approved' =>'0'));
            }else{
                $res = $this->admin_model->approveJob($job_id,array('approved' =>'1'));            
            }
            redirect('admin_jobs', 'refresh');
